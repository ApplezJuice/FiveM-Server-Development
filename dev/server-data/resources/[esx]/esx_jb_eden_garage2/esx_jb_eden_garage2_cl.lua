...
function OpenMenuGarage(garage, KindOfVehicle)
	...
	local elements = {
		{label = "Release vehicle from impound ("..Config.Price.."$)", value = 'return_vehicle'},
	}
...
function ListVehiclesMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		function(data, menu)
			local elem = {}
			table.insert(elem, {label ="Take vehicle out" , value = 'get_vehicle_out'})
				...
				function(data2, menu2)
					if data2.current.value == "get_vehicle_out" then
                        if (data.current.value.fourrieremecano) then
                            TriggerEvent('esx:showNotification', 'Your vehicle is in impound')
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'Vehicle stored in garage')
					else
						TriggerEvent('esx:showNotification', 'You cannot store this vehicle')
					end
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'This vehicle is in impound')
					else
						TriggerEvent('esx:showNotification', 'You cannot store this vehicle in impound')
					end
				end,trailerProps)
			else
				local vehicleProps  = ESX.Game.GetVehicleProperties(vehicle)
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'You cannot store this vehicle dans la fourrière')
					end
				end,vehicleProps)
			end
		else
		...
		function(data, menu)
			...
			elseif data.current.value ~= nil then
				...
				if not iscaronearth then
					...
					ESX.ShowNotification("You cannot take this vehicle out")
				end				
			end
		end,
		function(data, menu)
