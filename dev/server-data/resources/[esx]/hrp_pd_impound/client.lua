...
RegisterNUICallback('impound', function(data, cb)
	...
	local playerPos = GetEntityCoords( ped2, 1 )
            local inFrontOfPlayer = GetOffsetFromEntityInWorldCoords( ped2, 0.0, 5.0, 0.0 )
            local vehicle2 = GetVehicleInDirection( playerPos, inFrontOfPlayer )
...
Citizen.CreateThread(function ()
	while true do
		...
		if (IsControlJustReleased(1, 56) and (_XPlayer.job.name == "police" or _XPlayer.job.name == "mecano")) then
			print("This button was pressed for impound")
